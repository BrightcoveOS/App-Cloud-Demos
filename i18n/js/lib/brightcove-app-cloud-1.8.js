 ...
 */
   ...
   */ 
   ...
   /**
    * The <code>viewblur</code> event is triggered when a view loses focus, meaning that the user has switched to a different view.  When the app is closed, it does not trigger a <code>viewblur</code> event.
    * 
    * @example
...
( function( $ ) {
  ...
  * The fetchContentsOfURL also takes in an optional "options" object, which can have a property of encodingHint.  The purpose of this property is provide a hint for the encoding type of the requested file.
  * If no encodingHint is provided then the container will first try the encoding returned in the header of the requested file and if that fails then fallback to trying a set of standard encodings.
  * @example  
  ...
  */
  //* @param options The options param is an optional object that can be passed to the API.  Currently the only valid property is "encodingHint", which serves the purpose of informing the container as to what encoding the requested format is in.  If no encodingHint is provided then the container will first try the encoding returned in the header of the requested file and if that fails then fallback to trying a set of standard encodings.
  bc.device.fetchContentsOfURL = function( url, successCallback, errorCallback, options ) {
    var queryArgs
...
( function( bc, undefined ) {
  ...
  var _settings = undefined,
      ...
      _liveEvents = [],
      _errors = 0,
      _store_pendingevents_interval = undefined,
      _previous_pending_events = undefined;
